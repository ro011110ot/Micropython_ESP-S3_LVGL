...
class WeatherScreen:
    def __init__(self, mqtt):
        ...
        self.current_icon = ""
        self._icon_data = None  # PNG-Daten im Speicher halten!
        self._setup_ui()
        self.update_weather()
        self.update_time()
    ...
    def _setup_ui(self):
        ...
        # Status Card: PNG-Icon + Beschreibung
        ...
        self.hum_val = self._create_tile(125, 140, "Luftfeuchte", COLOR_PRIMARY)
        self.wind_val = self._create_tile(5, 215, "Wind", COLOR_ACCENT)
        self.pres_val = self._create_tile(125, 215, "Druck", 0x7B2FFF)
    ...
    def _load_icon(self, icon_code):
        """PNG von Dateisystem laden und als lv.image_dsc_t setzen."""
        ...
        try:
            ...
            # Referenz halten damit GC die Daten nicht löscht!
            self._icon_data = png_bytes

            img_dsc = lv.image_dsc_t(
                {
                    "data_size": len(png_bytes),
                    "data": memoryview(self._icon_data),
                }
            )

            self.weather_icon.set_src(img_dsc)
            self.weather_icon.invalidate()
            self.current_icon = icon_code
            print("Icon geladen:", icon_code)

        except OSError as e:
            print("Icon Fehler:", path, e)
